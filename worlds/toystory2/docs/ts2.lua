...
POST_CS_WAIT          = 180
DEATH_FREEZE_TIMER    = 0      -- countdown for the brief death-link freeze window
...
function check_token_collection(level_id,hover_id)
    ...
    -- corresponding location checks. We write the in-game token byte's check
    -- bits (Hamm's/Missing Toys/Race/Hidden) plus Boss (16) if present.
    local sa=SHARED_TOKENS_COLLECTED[hover_id]
    if sa then mainmemory.write_u8(sa, value & 0x1F) end
end
...
function update_traps(level)
    ...
    if dl_q>0 and is_death_link() and DEATH_FREEZE_TIMER==0 then
        mainmemory.write_u8(SHARED_DEATH_LINK,dl_q-1)
        -- Kill Buzz: write once and let the game play out the death. The death
        -- signal/sequence is health(0x0B221E)=255, state(0x0A155C)=255, and the
        -- death-animation byte (0x0A136E)=2. We also arm a SHORT freeze window so
        -- Buzz can't act during the death animation. We deliberately do NOT freeze
        -- "while 0x0A136E==2" indefinitely: that byte can read 2 on the
        -- death/respawn loading screen, where it blocked the Start/X button and
        -- left the player unable to continue past the load.
        mainmemory.write_u8(0x0B221E,255)
        mainmemory.write_u8(0x0A155C,255)
        mainmemory.write_u8(0x0A136E,2)
        DEATH_FREEZE_TIMER = 90  -- ~1.5s of freeze, then release no matter what
    end

    -- Brief, self-limiting death freeze. Only active for the countdown window
    -- right after WE applied a death-link kill, and only while genuinely in a
    -- playable level — never on the map or a loading screen.
    if DEATH_FREEZE_TIMER>0 then
        if level>=1 and level<=15 and mainmemory.read_u8(0x0A136E)==2 then
            mainmemory.write_u8(A.FREEZE,2)
            mainmemory.write_u8(A.INPUT,255)
        end
        DEATH_FREEZE_TIMER = DEATH_FREEZE_TIMER-1
        if DEATH_FREEZE_TIMER==0 then mainmemory.write_u8(A.FREEZE,0) end
    end
    ...
    if buzz_moved then
        for shared_addr,gadget in pairs(GADGETS) do
            if gadget.level==level then
                ...
                -- Re-assert usability EVERY frame for owned gadgets so they stay
                -- usable even after the game resets usability on level load.
                if owned==1 then assert_gadget_usable(gadget) end
            end
        end
    end
